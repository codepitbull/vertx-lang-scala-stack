...
class StompServerOptions(val asJava: io.vertx.ext.stomp.StompServerOptions) {
  ...
  def setSupportedVersions(value:scala.collection.mutable.Buffer[String]) = {
    asJava.setSupportedVersions(value)
    this
  }
...
object StompServerOptions {
  ...
  def fromJson(json: JsonObject):StompServerOptions = {
    if(json != null)
      new StompServerOptions(new StompServerOptionsJava(json))
    else
      null
  }
}
