...
an end handler. This is achieved using the `Sync.awaitEvent` method.
...
You do this with an instance of `HandlerReceiverAdaptor` which implements both
`Handler` and `Receiver`. You create an instance using
`Sync.streamAdaptor`.

You can set it as a normal handler and then use the methods on `Receiver` to receive
