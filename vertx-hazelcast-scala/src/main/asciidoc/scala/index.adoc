...
var mgr = io.vertx.spi.cluster.hazelcast.HazelcastClusterManager()
...
var hazelcastConfig = com.hazelcast.config.Config()

// Now set some stuff on the config (omitted)

var mgr = io.vertx.spi.cluster.hazelcast.HazelcastClusterManager(hazelcastConfig)
...
----
import io.vertx.scala.core.Vertx
var mgr = io.vertx.spi.cluster.hazelcast.HazelcastClusterManager(hazelcastInstance)
var options = VertxOptions()
  .setClusterManager(mgr)
...
`link:../../vertx-core/dataobjects.html#VertxOptions#setClusterHost(java.lang.String)[clusterHost]`.
