...
class OAuth2Auth(private val _asJava: io.vertx.ext.auth.oauth2.OAuth2Auth) {
  ...
  /**
    ...
    */
  def getToken(params: io.vertx.core.json.JsonObject, handler: io.vertx.core.AsyncResult[io.vertx.ext.auth.oauth2.AccessToken] => Unit): Unit = {
    _asJava.getToken(params, funcToHandler(handler))
  }
  ...
  /**
    ...
    * @return self
    */
  def api(method: io.vertx.core.http.HttpMethod, path: String, params: io.vertx.core.json.JsonObject, handler: io.vertx.core.AsyncResult[io.vertx.core.json.JsonObject] => Unit): io.vertx.scala.ext.auth.oauth2.OAuth2Auth = {
    _asJava.api(method, path, params, funcToHandler(handler))
    this
  }
...
object OAuth2Auth {
  ...
  def create(vertx: io.vertx.scala.core.Vertx, flow: io.vertx.ext.auth.oauth2.OAuth2FlowType, config: io.vertx.ext.auth.oauth2.OAuth2ClientOptions): io.vertx.scala.ext.auth.oauth2.OAuth2Auth = {
    OAuth2Auth.apply(io.vertx.ext.auth.oauth2.OAuth2Auth.create(vertx.asJava.asInstanceOf[io.vertx.core.Vertx], flow, config))
  }
